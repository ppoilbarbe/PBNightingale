...
class TranslationsBuildHook(BuildHookInterface):
    def initialize(self, version: str, build_data: dict[str, Any]) -> None:
        ...
        locale_dir = Path(self.root) / "src" / "pbnightingale" / "locale"
        for po_path in sorted(locale_dir.glob("*/LC_MESSAGES/pbnightingale.po")):
            lang = po_path.parts[-3]
            mo_path = po_path.with_suffix(".mo")
            with po_path.open("rb") as f:
                catalog = read_po(f, locale=lang, domain="pbnightingale")
            with mo_path.open("wb") as f:
                write_mo(f, catalog)
            build_data["force_include"][str(mo_path)] = (
                f"pbnightingale/locale/{lang}/LC_MESSAGES/pbnightingale.mo"
            )
