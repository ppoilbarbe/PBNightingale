...
# Placeholder until the project is actually published (see CLAUDE.md — no
# GitHub remote yet). The auto-update path is inert until then: it only
# runs from a PyInstaller-frozen executable (milestone 15), never from a
# source checkout or during tests.
_REPO = "ppoilbarbe/PBNightingale"
_API_URL = f"https://api.github.com/repos/{_REPO}/releases/latest"
_TIMEOUT = 30

# Mirrors the arch mapping to use in pbnightingale.spec (milestone 15).
_ARCH_MAP = {
    "x86_64": "x86_64",
...
def perform_auto_update() -> tuple[bool, str]:
    ...
    try:
        fd, tmp_name = tempfile.mkstemp(dir=dest_dir, prefix=".pbnightingale-update-")
        os.close(fd)
    ...
    return True, f"Updated to version {new_version}. Restart PBNightingale to use it."
