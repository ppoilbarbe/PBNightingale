"""Tests for pbnightingale.platform.auto_update — self-update via GitHub releases."""
...
from pbnightingale.platform import auto_update
...
class TestFindAsset:
    def test_matches_prefix_and_suffix(self):
        release = {
            "assets": [
                {"name": "pbnightingale-1.8.0-windows-x86_64.exe"},
                {"name": "pbnightingale-1.8.0-linux-x86_64"},
            ]
        }
        asset = auto_update._find_asset(release, "-linux-x86_64")
        assert asset["name"] == "pbnightingale-1.8.0-linux-x86_64"

    def test_returns_none_when_no_match(self):
        release = {"assets": [{"name": "pbnightingale-1.8.0-macos-arm64.zip"}]}
        assert auto_update._find_asset(release, "-linux-x86_64") is None

    def test_ignores_non_pbnightingale_assets(self):
...
class TestFetchLatestRelease:
    def test_parses_json_response(self, monkeypatch):
        data = _release("v1.8.0", [{"name": "pbnightingale-1.8.0-linux-x86_64"}])
        monkeypatch.setattr(
        ...
        assert release["assets"][0]["name"] == "pbnightingale-1.8.0-linux-x86_64"
...
class TestPerformAutoUpdateGuards:
    ...
    def test_already_up_to_date_skips_download(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            sys,
            "executable",
            str(tmp_path / f"pbnightingale-{__version__}-linux-x86_64"),
        )
        monkeypatch.setattr(
            auto_update,
            "_fetch_latest_release",
            lambda: {
                ...
                "assets": [
                    {
                        "name": f"pbnightingale-{__version__}-linux-x86_64",
...
class TestPerformAutoUpdateLinux:
    def test_success_replaces_executable_preserving_mode(self, monkeypatch, tmp_path):
        current = tmp_path / f"pbnightingale-{__version__}-linux-x86_64"
        ...
        monkeypatch.setattr(
            ...
            lambda: {
                ...
                "assets": [
                    {
                        "name": "pbnightingale-9.9.9-linux-x86_64",
                        "browser_download_url": "https://example.invalid/asset",
                    }
                ],
            },
...
class TestMacUpdaterAppRoot:
    def test_finds_enclosing_app_bundle(self, tmp_path):
        ...
        assert app_root == tmp_path / "PBNightingale.app"

    def test_returns_none_when_no_bundle_in_path(self, tmp_path):
        executable = tmp_path / "bin" / "pbnightingale-1.7.0"
        executable.parent.mkdir(parents=True)
        executable.touch()
...
class TestMacUpdaterReplace:
    def _make_app_zip(self, tmp_path, app_name="PBNightingale.app", content=b"NEW"):
        zip_path = tmp_path / "download.zip"
        with zipfile.ZipFile(zip_path, "w") as zf:
            zf.writestr(f"{app_name}/Contents/MacOS/pbnightingale", content)
        return zip_path

    def test_success_swaps_bundle_contents(self, tmp_path):
        app_root = tmp_path / "PBNightingale.app"
        (app_root / "Contents" / "MacOS").mkdir(parents=True)
        (app_root / "Contents" / "MacOS" / "pbnightingale").write_bytes(b"OLD")

        zip_path = self._make_app_zip(tmp_path)

        auto_update._MacUpdater().replace(app_root, zip_path)

        binary = app_root / "Contents" / "MacOS" / "pbnightingale"
        assert binary.read_bytes() == b"NEW"
        assert not (tmp_path / "PBNightingale.app.old").exists()
    ...
    def test_rejects_archive_without_exactly_one_app_bundle(self, tmp_path):
        ...
        assert (
            app_root / "Contents" / "MacOS" / "pbnightingale"
        ).read_bytes() == b"OLD"


class TestPerformAutoUpdateMacos:
    def test_missing_app_bundle_is_reported(self, monkeypatch, tmp_path):
        current = tmp_path / "bin" / f"pbnightingale-{__version__}-macos-arm64"
        ...
        monkeypatch.setattr(
            ...
            lambda: {
                ...
                "assets": [
                    {
                        "name": "pbnightingale-9.9.9-macos-arm64.zip",
                        "browser_download_url": "https://example.invalid/asset",
                    }
                ],
            },
    ...
    def test_success_replaces_app_bundle(self, monkeypatch, tmp_path):
        ...
        def fake_download(url, dest):
            with zipfile.ZipFile(dest, "w") as zf:
                zf.writestr(
                    "PBNightingale.app/Contents/MacOS/pbnightingale-9.9.9", b"NEW"
                )
