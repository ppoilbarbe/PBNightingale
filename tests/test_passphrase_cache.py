...
def test_stores_and_returns_the_passphrase():
    passphrase_cache.store("AAAA", Passphrase("s3cret"), 60)

    assert passphrase_cache.get("AAAA") == Passphrase("s3cret")
...
def test_expired_entry_is_forgotten(monkeypatch):
    ...
    passphrase_cache.store("AAAA", Passphrase("s3cret"), 60)  # expires at 160.0
...
def test_non_positive_ttl_is_a_noop():
    passphrase_cache.store("AAAA", Passphrase("s3cret"), 0)

    assert passphrase_cache.get("AAAA") is None


def test_empty_passphrase_is_a_noop():
    passphrase_cache.store("AAAA", Passphrase(""), 60)
...
def test_different_fingerprints_are_independent():
    passphrase_cache.store("AAAA", Passphrase("one"), 60)
    passphrase_cache.store("BBBB", Passphrase("two"), 60)

    assert passphrase_cache.get("AAAA") == Passphrase("one")
    assert passphrase_cache.get("BBBB") == Passphrase("two")
