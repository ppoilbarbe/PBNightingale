#!/usr/bin/env python3
"""
PO file inspection tool for PBNightingale.
...
LOCALE_DIR = Path(__file__).parent.parent / "src" / "pbnightingale" / "locale"
ALL_LANGS = sorted(
    p.name
    for p in LOCALE_DIR.iterdir()
    if p.is_dir() and (p / "LC_MESSAGES" / "pbnightingale.po").exists()
)


def load_catalog(lang: str):
    po_path = LOCALE_DIR / lang / "LC_MESSAGES" / "pbnightingale.po"
...
def main():
    ...
    for lang in langs:
        po = LOCALE_DIR / lang / "LC_MESSAGES" / "pbnightingale.po"
        if not po.exists():
