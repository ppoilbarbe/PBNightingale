#!/usr/bin/env python3
"""Sync icons in src/pbnightingale/resources/ from the PBIcons project.

Two kinds of files live in that directory, both sourced from PBIcons:

- The app icon files used by PyInstaller packaging and as the window icon:
  `pbnightingale.ico`, `pbnightingale.icns` and `pbnightingale.png`
  (`_APP_ICON_FILES` below) — listed explicitly since a raster/packaging
  icon isn't matched by the `*.svg` glob used for the toolbar/menu icons.
- Every `*.svg` already present in the resources directory (toolbar and
  menu action icons — Keys/Identities/Subkeys/Photos/Trust/Keyservers
  toolbars, Settings, Quit, About…).
...
updating, or the icon genuinely doesn't exist in PBIcons — GPG-specific
icons invented for this app (key-new, key-sign, key-revoke, trust-refresh,
server-search…) may simply not exist there yet.
...
Usage:
    ...
    python tools/update_icons.py quit.svg          # sync only the named file(s)
...
RESOURCES_DIR = ROOT / "src" / "pbnightingale" / "resources"

# App icon files (window icon + PyInstaller packaging), always synced
# regardless of what's already on disk — they can't be glob-discovered
# before their first sync, since only `pbnightingale.png` exists locally
# before packaging is set up (milestone 15).
_APP_ICON_FILES = (
    "pbnightingale.ico",
    "pbnightingale.icns",
    "pbnightingale.png",
...
_USER_AGENT = "pbnightingale-update-icons"
...
def _github_request(url: str) -> bytes:
    # url is always built from _GITHUB_TREE_API/_GITHUB_RAW_BASE constants
    # above, never from user input — always https.
...
def main() -> None:
    ...
    if "-h" in args or "--help" in args:
        sys.exit(__doc__)
    # Line-buffer stdout even when piped (e.g. `| tail`): otherwise a
    # pipe makes stdout fully block-buffered while stderr stays
    # unbuffered, so the unbuffered ERROR lines below can appear before
    # the "Local PBIcons checkout" line despite printing after it — easy
    # to misread as "the local checkout wasn't found" when it was.
